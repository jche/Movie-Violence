...
Though my data are not great, they do not display any glaring discrepancies with Dahl and DellaVigna's data either. As such, I proceed to calculate expected exposure to violence using a movie's MPAA rating and genre.
...
ggplot(foo, aes(x=MPAA_Rating, y=Violence)) +
  geom_jitter(aes(color=Genre, shape=Genre)) +
  scale_shape_manual(values=1:nlevels(foo$Genre)) +
...
daily_exposure <- movie %>%
  ...
  summarise(tickets_tot = sum(Tickets),
            ...
            tickets_to_aggexpviol = sum(Tickets*Exp_Violence))
```
